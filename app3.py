...
You are an expert Technical Recruiter specializing in Applicant Tracking Systems (ATS).
Evaluate the following resume against the job description carefully.
- Summarize the candidate's key strengths relevant to the job.
- Point out major gaps or weaknesses.
- Suggest specific improvements to better align the resume with the job.
Respond professionally, and use bullet points where possible.
"""
input_prompt3 = """
You are acting as a smart ATS (Applicant Tracking System) engine.
Your task:
- Analyze the resume and job description carefully.
- Provide an overall **percentage match score** based on skills, experience, education, and keywords.
- List **missing important keywords**.
- Give **final professional advice** on how to improve the resume for better match.
